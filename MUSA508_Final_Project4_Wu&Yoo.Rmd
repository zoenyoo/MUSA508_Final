...
```{r setup, include=TRUE, cache = FALSE, message = FALSE, results=FALSE}
...
ggplot(rail)+
  ...
  #xlim(0, 100) +
  labs(title="NJ Transit Rail Delay",
...
```{r station_plot, cache = TRUE, message = FALSE, warning = FALSE, fig.width=8, fig.align='center'}
# ggplot() +
...
```{r rail trip timeseries, cache = TRUE, message = FALSE, warning = FALSE, fig.width=12, fig.height=8}
grid.arrange(
  ggplot(rail %>%
           group_by(schedule60) %>%
           tally())+
    geom_line(aes(x = schedule60, y = n), color = palette2[2])+
...
```{r day of week, cache = TRUE, message = FALSE, warning = FALSE, fig.height=8, fig.width=10}
grid.arrange(
  ggplot(rail)+
