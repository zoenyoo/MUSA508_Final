...
In Figure 3.6, We noticed there are some time processes of train delay by day of the week and time of the day, so we engineered new variables of each train record: `weekday` and `time_of_day`.
...
Model D is good at predicting trains delayed within 5, 10, and 20 mins. 
...
Of each actual delay time category, our model successfully capture 46%, 58%, 59% and 9% of delayed trains.

```{r validation table, cache=TRUE, warning = FALSE, message = FALSE, fig.width=16}
