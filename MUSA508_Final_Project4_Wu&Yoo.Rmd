...
```{r images, echo=FALSE, out.width="100%", out.height="100%", fig.cap="Fig 2.0 Data Structure",fig.show='hold',fig.align='center'}
...
From Figure 3.6, we noticed there are some time processes of train delay by day of the week and time of the day, so we engineered new variables of each train record: `weekday` and `time_of_day`.
...
There are relationships between delay and time lag, as shown in the scatter plots in Figure 3.7.
...
For our models, we train on the first five weeks of our dataset and test on the last three. 
...
We have developed four different models with increasing levels of detail. The first regression includes time and weather data and the second includes spatial and weather data; the third model combines the two. The fourth, and most detailed model, includes the aforementioned data, as well as data on lag time and stop sequencing. As can be seen in Table 4.1, the fourth model has the highest R^2^ value, so we will continue to test with this model.
...
In Figure 4.2, it is clear that mean absolute error is also the least for the fourth, most detailed model.
...
```{r validation_space, cache=TRUE, warning = FALSE, message = FALSE, fig.width=16}
ggplot()+
...
Model D is generally accurate when predicting trains delayed within 5, 10, and 20 mins (shown in Figure 4.5). 
...
Of each actual delay time category, our model successfully captures 57%, 71%, and 73% of delayed trains for categories under 20 minutes, but only 9% of trains with delay over 20 minutes.
...
For cross-validation, we train on day of the week, training one day against the six other days. In the results from the cross-validation process (shown in Figure 4.6), daily mean absolute error is mostly low, with mean MAE being 0.6 minutes and maximum daily MAE at just under two minutes.
